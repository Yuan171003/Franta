% Compile with Tectonic or XeLaTeX; no external bibliography files are needed.
% With XeLaTeX, run twice to resolve the bibliography hyperlinks.
...
% Use the supported U+2216 glyph for the set-difference operator.
\AtBeginDocument{\let\setminus\smallsetminus}
\usepackage{microtype}
\usepackage{xurl}
\usepackage[unicode,colorlinks=true,linkcolor=blue,citecolor=blue,urlcolor=blue]{hyperref}
\hypersetup{
  pdftitle={Derived Equivalence and Chow Motives of Threefolds of Positive Kodaira Dimension},
  pdfauthor={Franta}
}
...
\title{Derived Equivalence and Chow Motives\\of Threefolds of Positive Kodaira Dimension}
\author{Franta}
...
\noindent \textbf{Keywords:} Chow motives; derived equivalence; threefolds; Kodaira dimension; Iitaka fibration; Fourier--Mukai transforms.

\end{abstract}

\section{Introduction}

Orlov's conjecture predicts that derived-equivalent smooth projective varieties have isomorphic Chow motives with rational coefficients \cite{Orl05}. A Fourier--Mukai equivalence provides mutually inverse correspondences after allowing components of different codimensions and Tate twists. Recovering an isomorphism of ordinary Chow motives requires controlling the contributions that change degree. For surfaces, this can be done using the structure of their Chow--Künneth decompositions; see \cite[Theorem 1.1]{FV21}.

The purpose of this paper is to establish the following three-dimensional case.

\noindent \textbf{Theorem 1.1.} Let $X$ and $Y$ be smooth connected projective varieties of dimension three over $\mathbb C$. Suppose that there is an exact $\mathbb C$-linear equivalence
...
\section{Boundary correspondences and cancellation}
...
\subsection{Correspondences with proper support on both sides}

\noindent \textbf{Lemma 2.1.} Let $X$ and $Y$ be smooth projective complex threefolds. If $z\in\mathrm{CH}^3(X\times Y)$ has a cycle representative supported on $D\times E$, where $D\subsetneq X$ and $E\subsetneq Y$ are closed, then

\begin{equation*}
z\in\mathcal I(h(X),h(Y)).
\end{equation*}

\noindent \emph{Proof.} Enlarge $D$ and $E$ to divisors, and resolve their reduced irreducible components. We obtain smooth projective surfaces $S_i,T_j$ and proper maps
...
Choose the surface Chow--Künneth decompositions of \cite[Theorem 1.4]{FV21}. The component of $\delta$ from $h^a(S)$ to $h^b(T)(-1)$ vanishes unless

\begin{equation*}
b+2\leq a\leq b+3;
\end{equation*}

this is \cite[Theorem 1.4(ii), equation (3)]{FV21} with source dimension two and twist $-1$. Thus the only possible pairs $(a,b)$ are
...
\subsection{The structure of the boundary category}

\noindent \textbf{Lemma 2.2.} The realization functor on $\mathcal B_3$ is full. Its kernel is a square-zero ideal. For each $P\in\mathcal B_3$, the ring $\operatorname{End}(P)$ has a square-zero ideal with semisimple Artinian quotient. Moreover,
...
\noindent \emph{Proof.} First consider finite direct sums of the generators (2.1), ordered by their realization degrees $2,3,4$. Applying the pointed-curve Chow projectors to divisor and fundamental classes shows that the only potentially nonzero off-diagonal morphisms are
...
\noindent \textbf{Lemma 2.3 (cancellation).} Let $M,N$ be rational Chow motives and let $P\in\mathcal B_3$. Then
...
\noindent \emph{Proof.} Put $R=\operatorname{End}(P)$. We first establish the following stable-range property:
...
\subsection{Lifting an isomorphism modulo the boundary ideal}

\noindent \textbf{Proposition 2.4.} Let $M,N$ be rational Chow motives over $\mathbb C$. Suppose that there are morphisms $f:M\to N$ and $g:N\to M$ with
...
\noindent \emph{Proof.} Factor $gf-1_M=sr$ through some $P\in\mathcal B_3$, where $r:M\to P$ and $s:P\to M$. Define
...
\section{Spreading generic correspondences and changing smooth models}

\subsection{Generic fibres over a common base}

\noindent \textbf{Proposition 3.1.} Let $X$ and $Y$ be smooth connected projective complex threefolds with dominant morphisms
...
\noindent \emph{Proof.} Put $b=\dim B$ and $r=3-b$. Choose inverse correspondences defined over $K$,
...
For smooth total spaces projective over $B$, relative correspondences have an associative composition that preserves support over $B$ \cite[\S{}3.1]{CH07}. Explicitly, take the external product, apply refined Gysin pullback along the diagonal of the smooth absolute middle variety, and then use proper pushforward. The dimension shift is the dimension of that middle variety, so in the present case
...
A rational equivalence over $K$ is represented by finitely many subvarieties and rational functions. Spreading these data shows that both defects in (3.2) vanish after a common shrinking of $B$. By Chow localization \cite[Proposition 1.8]{Ful98}, they therefore come from cycles supported over a proper closed subset $Z\subsetneq B$. For example, the first lies in the image of
...
\noindent \textbf{Lemma 3.2.} Birational smooth projective complex threefolds have isomorphic Chow motives modulo $\mathcal I$.

\noindent \emph{Proof.} First let $p:Z\to X$ be a projective birational morphism between smooth projective threefolds. The graph correspondences satisfy
...
\noindent \textbf{Corollary 3.3.} If two smooth projective complex threefolds are birational and have degreewise isomorphic rational Hodge structures, their rational Chow motives are isomorphic.

\noindent \emph{Proof.} Combine Lemma 3.2 with Proposition 2.4. $\square$

\section{Canonical free loci and Fourier--Mukai support}

Throughout this section, $X,Y$ and $\Phi$ satisfy the hypotheses of Theorem 1.1. By representability \cite{Orl97}, choose Fourier--Mukai kernels $P\in D^b(X\times Y)$ and $Q\in D^b(Y\times X)$ for $\Phi$ and its inverse.

\subsection{Compatibility with actual canonical-section maps}
...
We use the compatibility at the level of section morphisms established in \cite[Proposition 4.1, Lemmas 4.2--4.3, and Corollary 4.4]{Tod06}. Namely, for a section $s\in H^0(X,\omega_X^{\otimes m})$, (4.1) identifies multiplication by $p_X^*s$ on $P$ with multiplication by $p_Y^*\rho(s)$. Consequently, for every $E\in D^b(X)$, the morphism
...
\subsection{Free loci on good minimal models}

\noindent \textbf{Lemma 4.1.} There are projective $\mathbb Q$-factorial terminal minimal models $T_X,T_Y$, closed subsets
...
\noindent \emph{Proof.} Run a $K$-negative threefold minimal model program on each variety. The existence and termination results in dimension three give projective $\mathbb Q$-factorial terminal minimal models, and abundance makes their canonical divisors semiample; see \cite{FA92} and \cite{Kaw92}. The Mori-fibre-space outcome is excluded by positive Kodaira dimension. Canonical rings are preserved by the program.
...
Terminal threefold singularities are isolated \cite[Proposition 2.7]{Kol90}. Since $\dim B>0$, their images do not contain the generic point of $B$. Generic smoothness, followed by properness to remove the image of the nonsmooth locus, therefore gives smooth families over a nonempty open of $B$. Connected fibres and normality of $B$ give $f_*\mathcal O_{T_X}=\mathcal O_B$ and similarly for $g$. Consequently the smooth generic fibres are geometrically connected and hence geometrically integral. Their dimension is $3-\dim B$. $\square$

\subsection{Properness on the free loci}

\noindent \textbf{Lemma 4.2.} In the notation of Lemma 4.1, put
...
\noindent \emph{Proof.} Suppose that $x\in X^\circ$ and $y\notin Y^\circ$. Choose $s\in H^0(X,mK_X)$ with $s(x)\neq0$. Then $\rho(s)(y)=0$. Apply the section compatibility of \S{}4.1 to the derived residue fibre of $P$ at $(x,y)$. On this fibre, multiplication by $s$ is invertible and multiplication by $\rho(s)$ is zero. The fibre must therefore vanish. Derived Nakayama implies $(x,y)\notin\operatorname{Supp}(P)$. Interchanging the factors gives
...
\subsection{An omitted horizontal curve forces birationality}

\noindent \textbf{Proposition 4.3.} Suppose that $\dim B=1$. If either $C_X$ or $C_Y$ dominates $B$, then $X$ and $Y$ are birational.

\noindent \emph{Proof.} Suppose first that $C_X$ dominates $B$. Write
...
The object is nonzero since $\Phi$ is an equivalence. Toda's point-support criterion \cite[Lemma 7.3]{Tod06} now implies that $X$ and $Y$ are birational. If $C_Y$ dominates $B$, apply the same argument to the inverse kernel $Q$. $\square$
...
\section{The proper generic fibres}
...
\subsection{Canonical-section localization}

\noindent \textbf{Proposition 5.1.} There is an exact $K$-linear equivalence
...
\noindent \emph{Proof.} Let $\mathcal T_X$ be the thick subcategory of $\operatorname{Perf}(X)$ generated by the cones of all multiplication maps
...
The compatibility of the actual section maps in \S{}4.1 gives
...
where $(-)^{\natural}$ denotes idempotent completion. This follows from \cite[Theorem 2.1]{Nee92}; the supported compact-generation and compactness statements are \cite[Tags 0A9A and 0A9B]{Stacks}, and perfect extension up to a summand is \cite[Tag 09IM]{Stacks}.

The ring $R$ is a regular integral finitely generated $\mathbb C$-algebra, its closed points are $\mathbb C$-rational, and $(T_X)_U\to\operatorname{Spec}R$ is smooth and separated. Thus \cite[Corollary 2.6]{Mor25} applies and gives
...
\subsection{Motives in dimensions at most two over the function field}

\noindent \textbf{Lemma 5.2.} Let $V$ and $W$ be smooth projective geometrically integral varieties of the same dimension at most two over a characteristic-zero field $K$. If $D^b(V)\simeq D^b(W)$ by an exact $K$-linear equivalence, then
...
\noindent \emph{Proof.} In dimension two, the assertion is \cite[Theorem 1.1]{FV21}, which is stated over the given base field. In dimension zero, geometric integrality gives $V=W=\operatorname{Spec}K$.
...
Grothendieck--Riemann--Roch applied to convolution gives the codimension-one identities \cite{Orl05}
...
\noindent \textbf{Corollary 5.3.} In the situation of Proposition 5.1, there are inverse rational Chow correspondences between $G_X$ and $G_Y$ defined over $K=\mathbb C(B)$.

\noindent \emph{Proof.} Their common dimension is $3-\dim B\in\{0,1,2\}$. Apply Lemma 5.2 to Proposition 5.1. $\square$

\section{Proof of the main theorem}

\emph{Proof of Theorem 1.1.} Derived equivalence of smooth projective complex threefolds preserves their rational Hodge structures degree by degree \cite[Theorem 2(a)]{ACMV19}. Thus
...
\noindent \textbf{Remark 6.1.} The Hodge comparison is used only for the original derived-equivalent pair $X,Y$. The smooth graph resolutions need not be derived equivalent or have isomorphic Hodge structures. Similarly, ordinary smooth Chow motives are assigned only to smooth projective varieties; the singular minimal models serve to construct the common base and its proper generic fibres.

\noindent \textbf{Remark 6.2.} The horizontal case in Proposition 4.3 is necessary even for elementary birational modifications. If $S$ is a projective K3 surface, $p\in S$, and $C$ is a curve of genus at least two, then
...
\noindent \textbf{Remark 6.3.} Positive Kodaira dimension enters through the inequality $\dim B>0$, which reduces the proper generic fibres to dimension at most two. When $\kappa(X)=0$, the Iitaka base is a point, so this reduction does not address the general three-dimensional case. The support calculation of Lemma 2.1 is also specific to dimension three: after resolving the two boundary divisors, it involves divisor classes on a product of surfaces.

\begin{thebibliography}{ACMV19}
\setlength{\itemsep}{0.6\baselineskip}

\bibitem[ACMV19]{ACMV19} Jeffrey D. Achter, Sebastian Casalaina-Martin, and Charles Vial, \emph{Derived equivalent threefolds, algebraic representatives, and the coniveau filtration}, Mathematical Proceedings of the Cambridge Philosophical Society \textbf{167} (2019), no. 1, 123--131. \href{https://doi.org/10.1017/S0305004118000221}{DOI}; \href{https://arxiv.org/abs/1704.01902}{arXiv:1704.01902}. The degreewise rational Hodge comparison used here is Theorem 2(a).

\bibitem[CH07]{CH07} Alessio Corti and Masaki Hanamura, \emph{Motivic decomposition and intersection Chow groups II}, Pure and Applied Mathematics Quarterly \textbf{3} (2007), no. 1, 181--203. \href{https://doi.org/10.4310/PAMQ.2007.v3.n1.a6}{DOI}; \href{https://www.ma.imperial.ac.uk/~acorti/download/md-ich.pdf}{author manuscript}. The construction of relative correspondences used here is in \S{}3.1.

\bibitem[FA92]{FA92} János Kollár (ed.), \emph{Flips and abundance for algebraic threefolds: A summer seminar at the University of Utah (Salt Lake City, 1991)}, Astérisque \textbf{211}, Société Mathématique de France, 1992. \href{https://numdam.org/item/AST_1992__211__1_0/}{Numdam}. See the overview in Chapter 1 and the threefold minimal model and abundance results developed in the volume.

\bibitem[Ful98]{Ful98} William Fulton, \emph{Intersection Theory}, second edition, Ergebnisse der Mathematik und ihrer Grenzgebiete, 3. Folge, vol. 2, Springer-Verlag, Berlin, 1998. \href{https://doi.org/10.1007/978-1-4612-1700-8}{DOI}. We use Chow localization, refined Gysin operations and their compatibilities, and Grothendieck--Riemann--Roch.

\bibitem[FV21]{FV21} Lie Fu and Charles Vial, \emph{A motivic global Torelli theorem for isogenous K3 surfaces}, Advances in Mathematics \textbf{383} (2021), article 107674. \href{https://arxiv.org/abs/1907.10868}{arXiv:1907.10868}; \href{https://irma.math.unistra.fr/~lfu/articles/DerivedEqK3.pdf}{author manuscript}. See Theorem 1.1 for surfaces over the given field, Theorem 1.4 and equation (3) for the Chow--Künneth statements, and Proposition 1.5 for degree-one curve motives.

\bibitem[Kaw92]{Kaw92} Yujiro Kawamata, \emph{Abundance theorem for minimal threefolds}, Inventiones Mathematicae \textbf{108} (1992), 229--246. \href{https://doi.org/10.1007/BF02100604}{DOI}.

\bibitem[Kol90]{Kol90} János Kollár, \emph{Minimal models of algebraic threefolds: Mori's program}, Séminaire Bourbaki, exp. no. 712, Astérisque \textbf{177--178} (1990), 303--326. \href{https://www.numdam.org/item/SB_1988-1989__31__303_0/}{Numdam}. See Proposition 2.7 for isolatedness of terminal threefold singularities.

\bibitem[Mor25]{Mor25} Hayato Morimura, \emph{Categorical generic fiber}, Journal of Algebra \textbf{667} (2025), 75--108. \href{https://doi.org/10.1016/j.jalgebra.2024.12.013}{DOI}; \href{https://arxiv.org/abs/2111.00239}{arXiv:2111.00239}. We use Corollary 2.6 under the hypotheses stated at the beginning of \S{}2.

\bibitem[Nee92]{Nee92} Amnon Neeman, \emph{The connection between the $K$-theory localization theorem of Thomason, Trobaugh and Yao and the smashing subcategories of Bousfield and Ravenel}, Annales Scientifiques de l'École Normale Supérieure (4) \textbf{25} (1992), no. 5, 547--566. \href{https://doi.org/10.24033/asens.1659}{DOI}. See Theorem 2.1 for the localization statement on compact objects.

\bibitem[Orl97]{Orl97} Dmitri Orlov, \emph{Equivalences of derived categories and K3 surfaces}, Journal of Mathematical Sciences \textbf{84} (1997), no. 5, 1361--1381. \href{https://arxiv.org/abs/alg-geom/9606006}{arXiv:alg-geom/9606006}. See Theorem 2.18 for representability of equivalences.

\bibitem[Orl05]{Orl05} Dmitri Orlov, \emph{Derived categories of coherent sheaves and motives}, Russian Mathematical Surveys \textbf{60} (2005), no. 6, 1242--1244. \href{https://arxiv.org/abs/math/0512620}{arXiv:math/0512620}. We use the Mukai-vector composition identity obtained from Grothendieck--Riemann--Roch.

\bibitem[Stacks]{Stacks} The Stacks Project Authors, \emph{The Stacks Project}. \href{https://stacks.math.columbia.edu/tag/0A9A}{Tag 0A9A}, \href{https://stacks.math.columbia.edu/tag/0A9B}{Tag 0A9B}, and \href{https://stacks.math.columbia.edu/tag/09IM}{Tag 09IM}. Accessed 14 September 2026.

\bibitem[Tod06]{Tod06} Yukinobu Toda, \emph{Fourier--Mukai transforms and canonical divisors}, Compositio Mathematica \textbf{142} (2006), no. 4, 962--982. \href{https://arxiv.org/abs/math/0312015}{arXiv:math/0312015}. We use Proposition 4.1, Lemmas 4.2--4.3, Corollary 4.4, and Lemma 7.3; the proof of Theorem 7.5, Step 1, contains the related free-locus comparison in Kodaira dimension two.
